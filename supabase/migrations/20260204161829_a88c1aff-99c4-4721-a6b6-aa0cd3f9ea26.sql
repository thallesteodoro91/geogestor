-- Update get_financial_dashboard_metrics to include tipo (classificacao) in custos_por_categoria
CREATE OR REPLACE FUNCTION public.get_financial_dashboard_metrics(p_data_inicio date DEFAULT NULL::date, p_data_fim date DEFAULT NULL::date)
 RETURNS json
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  SELECT json_build_object(
    ...
    -- Margem por Serviço com RECEITA (Top 8 para matriz de eficiência)
    ...
    -- Custos por Categoria com TIPO (classificação) para Treemap
