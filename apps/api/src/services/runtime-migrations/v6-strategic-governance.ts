...
export const STRATEGIC_GOVERNANCE_MIGRATION = {
  version: 6,
  name: 'strategic-governance-2026-07-31'
} as const;
...
export async function ensureStrategicGovernance(client: Client) {
  if (!(await hasColumn(client, 'objetivos_estrategicos', 'ordem'))) {
    await client.execute('ALTER TABLE objetivos_estrategicos ADD COLUMN ordem INTEGER DEFAULT 0 NOT NULL');
  }
  ...
  const indexes = [
    'CREATE INDEX IF NOT EXISTS idx_objetivos_estrategicos_pilar_ordem ON objetivos_estrategicos(pilar_id, ordem)',
    'CREATE INDEX IF NOT EXISTS idx_decisoes_estrategicas_ciclo_status_prazo ON decisoes_estrategicas(ciclo_id, status, prazo)',
    'CREATE INDEX IF NOT EXISTS idx_decisoes_estrategicas_checkin ON decisoes_estrategicas(checkin_id)',
    'CREATE INDEX IF NOT EXISTS idx_decisoes_estrategicas_objetivo ON decisoes_estrategicas(objetivo_id)',
    'CREATE INDEX IF NOT EXISTS idx_snapshots_estrategicos_ciclo_captura ON snapshots_estrategicos(ciclo_id, capturado_em)',
    'CREATE UNIQUE INDEX IF NOT EXISTS idx_snapshots_estrategicos_checkin_unique ON snapshots_estrategicos(checkin_id)'
  ];
