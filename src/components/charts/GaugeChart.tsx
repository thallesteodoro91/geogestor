...
export const GaugeChart = ({ value, max, title, subtitle, description = "Indica o quanto da meta foi atingido. Valores acima de 80% são considerados ótimos, entre 50-80% são moderados, e abaixo de 50% requerem atenção." }: GaugeChartProps) => {
  ...
  const showEmptyState = max <= 0;

  return (
    <Card className="bg-gradient-to-br from-card to-card/50 border-border/50 interactive-lift">
      ...
      <CardContent className="flex flex-col items-center pt-2">
        {showEmptyState ? (
          <div className="flex flex-col items-center justify-center h-28 mb-4 text-center">
            <p className="text-muted-foreground text-sm">
              Cadastre custos e despesas fixas para calcular o ponto de equilíbrio.
            </p>
          </div>
        ) : (
        ...
        <div className="text-center">
          {showEmptyState ? (
            <>
              ...
              <p className="text-sm text-muted-foreground/80">Meta não definida</p>
            </>
          ) : (
            <>
              ...
              <p className="text-sm text-muted-foreground/80">
                {percentage.toFixed(1)}% de {max.toLocaleString('pt-BR')}
              </p>
