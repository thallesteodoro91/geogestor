      ...
      <form onSubmit={handleSubmit} className="space-y-5" noValidate>
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
          <FormField htmlFor="license-tipoLicenca" label="Tipo da licença" required error={errors.tipoLicenca}>
            <FormSelect id="license-tipoLicenca" name="tipoLicenca" autoComplete="off" value={form.tipoLicenca} onChange={(event) => update('tipoLicenca', event.target.value)} aria-invalid={Boolean(errors.tipoLicenca)} className={cn(fieldClass, 'geo-native-select')}>
          ...
          <FormField htmlFor="license-numero" label="Número da licença" required error={errors.numero}>
            <input id="license-numero" name="numero" type="text" autoComplete="off" spellCheck={false} value={form.numero} onChange={(event) => update('numero', event.target.value)} placeholder="Ex.: LAO 1234/2026" aria-invalid={Boolean(errors.numero)} className={fieldClass} />
          </FormField>
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          <FormField htmlFor="license-orgao" label="Órgão ambiental" required error={errors.orgao}>
            <input id="license-orgao" name="orgao" type="text" autoComplete="organization" value={form.orgao} onChange={(event) => update('orgao', event.target.value)} placeholder="Ex.: IMA, IBAMA" aria-invalid={Boolean(errors.orgao)} className={fieldClass} />
          </FormField>
          <FormField htmlFor="license-status" label="Status inicial">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          <FormField htmlFor="license-dataVencimento" label="Data de vencimento" required error={errors.dataVencimento}>
            <DatePickerField id="license-dataVencimento" name="dataVencimento" autoComplete="off" min={form.dataEmissao || undefined} value={form.dataVencimento} onChange={(event) => update('dataVencimento', event.target.value)} aria-invalid={Boolean(errors.dataVencimento)} className={fieldClass} />
          </FormField>
        </div>
