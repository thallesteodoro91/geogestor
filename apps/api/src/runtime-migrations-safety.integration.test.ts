...
test('migração de orçamento legado é idempotente em execuções repetidas', async () => {
  ...
  await client.execute("UPDATE schema_migrations SET status = 'failed' WHERE version = 7");
  ...
  const migrations = await client.execute('SELECT version, status FROM schema_migrations WHERE version IN (1, 2, 3, 4, 5, 6, 7) ORDER BY version');
  ...
  assert.deepEqual(migrations.rows.map((row) => [Number(row.version), row.status]), [[1, 'success'], [2, 'success'], [3, 'success'], [4, 'success'], [5, 'success'], [6, 'success'], [7, 'success']]);
  assert.equal(Number(userVersion.rows[0]?.user_version), 7);
