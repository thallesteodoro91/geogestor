...
test('fast path exige ledger success e user_version coincidentes', async () => {
  ...
  await client.execute({
    sql: 'UPDATE schema_migrations SET started_at = ? WHERE version = 10',
    args: [sentinel]
  });
  const fast = await runRuntimeMigrations();
  assert.equal(fast.fastPath, true);
  assert.equal(
    (await client.execute('SELECT started_at FROM schema_migrations WHERE version = 10')).rows[0]?.started_at,
    sentinel
  );

  for (const status of ['running', 'failed']) {
    await client.execute({
      sql: 'UPDATE schema_migrations SET status = ?, started_at = ? WHERE version = 10',
      args: [status, sentinel]
    });
    const recovered = await runRuntimeMigrations();
    assert.equal(recovered.fastPath, false);
    const ledger = (await client.execute(
      'SELECT status, started_at FROM schema_migrations WHERE version = 10'
    )).rows[0];
  ...
  await client.execute({
    sql: "UPDATE schema_migrations SET status = 'success', started_at = ? WHERE version = 10",
    args: [sentinel]
  });
